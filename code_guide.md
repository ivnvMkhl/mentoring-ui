...
- Тип пропсов декларируется через дженерик `React.FC<>` исключения когда нужно создать дженерик
- Пропсы вытаскиваем деструктуризацией из первого параметра компоненты
- Используем именованный экспорт, использовать `export default` запрещено
- Декларируем экспорты внизу компоненты
- Для компоненты используем стрелочную функцию а не `function`
